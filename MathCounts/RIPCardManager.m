...
#import "RIPCardManager.h"
#import "RIPCard.h"

@interface RIPCardManager ()
...
+ (instancetype)sharedManager
{
    ...
    static RIPCardManager *sharedManager = nil;
    
    //Creates thread-safe singleton; has to do with multithreading; research this more
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ sharedManager = [[self alloc] initPrivate]; });
...
- (instancetype)initPrivate
{
    ...
    self = [super init];
    if (self) {
        NSString *path = [self testArchivePath];
        self.privateTestStore = [NSKeyedUnarchiver unarchiveObjectWithFile:path];
...
- (instancetype)init
{
    ...
    @throw [NSException exceptionWithName:@"Singleton"
                                   reason:@"Use + [RIPCardManager sharedManager]"
                                 userInfo:nil];
    return nil;
}
...
{
    NSString *path = [self testArchivePath];
    return [NSKeyedArchiver archiveRootObject:self.testStore toFile:path];
}
